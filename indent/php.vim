...
let s:blockstart = '\%(\%(\%(}\s*\)\=else\%(\s\+\)\=\)\=if\>\|else\>\|while\>\|switch\>\|for\%(each\)\=\>\|declare\>\|class\>\|interface\>\|abstract\>\|try\>\|catch\>\)' " XXX 20071103 removed \|[|&] at end
...
function! GetPhpIndent()
    ...
    "let unstated  = '\%(^\s*'.s:blockstart.'.*[^;]\|\%(//.*\)\@<!\<e'.'lse\>\)'.endline " XXX changed on 20071103
    ...
    elseif cline =~ '^\s*)\=\s*{' " XXX changed on 2007-11-03 to fix a issue when a closing paren is preceding an openning brace
    ...
    elseif last_line =~# unstated && cline !~ '^\s*);\='.endline " XXX 20071103 Previously '^\s*{\|^\s*);\='.endline : Not sure if the ^\s*{\| part is useful since it was tested just before...
	...
	while 1
		...
		if one_ahead_indent == two_ahead_indent || last_line_num < 1
		    ...
		    " the same indent...
		    if previous_line =~# '\%(;\|^\s*}\)'.endline || last_line_num < 1 " XXX changed on 2007 - 11 - 04 (previous_line =~# '[;}]'.endline)
			break
		    endif
		endif
	    endif
